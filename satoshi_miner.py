...
class GpuSolver:
    ...
    def __init__(self, cfg):
        ...
        self.batch = int(cfg.get("gpu_batch_size", 32_000_000))
...
def main_headless():
    """Headless (CLI) mode — no display required. Logs to stdout."""
    multiprocessing.freeze_support()
    cfg = load_yaml_config()

    pk = os.environ.get("PRIVATE_KEY", "") or cfg.get("private_key", "")
    if not pk or pk == "YOUR_PRIVATE_KEY_HERE":
        print("[FATAL] No private key found. Set it in config.yaml or PRIVATE_KEY env var.")
        sys.exit(1)
    if not pk.startswith("0x"):
        pk = "0x" + pk
    cfg["_pk"] = pk
    ...
    def on_win(epoch):
        print(f"\n{'='*50}\n  >>> BLOCK MINED! epoch={epoch} <<<\n{'='*50}", flush=True)

    def on_lose():
        pass

    def on_stats(s):
        print(f"  [{s.get('mode','CPU')}] sent={s.get('sent',0)} won={s.get('won',0)} "
              f"lost={s.get('lost',0)} win_rate={s.get('win_rate',0):.0f}% "
              f"uptime={s.get('elapsed',0):.0f}s", flush=True)

    print(f"Satoshi Miner v3.0 — Headless Mode")
    print(f"  Solver: {cfg.get('solver', 'cpu').upper()}")
    print(f"  Dry run: {bool(cfg.get('dry_run', False))}")
    print(f"  Address: {addr}")
    print()

    miner = ReactiveMiner(cfg, on_log=on_log, on_win=on_win, on_lose=on_lose, on_stats=on_stats)
    ...
    miner.start()
    print("Mining started. Press Ctrl+C to stop.", flush=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Satoshi (SAT) Token Miner v3.0")
    parser.add_argument("-H", "--headless", action="store_true",
                        help="Run in headless (CLI) mode, no GUI required")
